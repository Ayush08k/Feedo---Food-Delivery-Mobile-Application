    ...
    if (items.length === 0) {
        return (
            <SafeAreaView className="flex-1 bg-[#121212] justify-center items-center">
                <Text className="text-[#A0A0A0] text-xl mb-4">{t('cart.empty')}</Text>
                <TouchableOpacity
                    onPress={() => navigation.navigate('RestaurantsList')}
                    className="bg-[#1DB954] px-6 py-3 rounded-full"
                >
                    <Text className="text-black font-bold">{t('cart.browseRestaurants')}</Text>
                </TouchableOpacity>
            </SafeAreaView>
        );
    }
    ...
    return (
        <SafeAreaView className="flex-1 bg-[#121212]">
            <View className="px-4 py-4 border-b border-[#333]">
                <Text className="text-white text-2xl font-bold">{t('cart.title')}</Text>
            </View>
                    ...
                    <View key={item.id} className="flex-row justify-between items-center mb-6 bg-[#1E1E1E] p-4 rounded-xl border border-[#333]">
                        ...
                        <View className="items-end">
                            ...
                            <TouchableOpacity onPress={() => removeFromCart(item.id)}>
                                <Text className="text-red-500 text-sm mt-1">{t('cart.remove')}</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                ))}
            ...
            <View className="p-4 bg-[#1E1E1E] rounded-t-3xl border-t border-[#333]">
                <View className="flex-row justify-between mb-4">
                    <Text className="text-[#A0A0A0] text-lg">{t('cart.total')}</Text>
                ...
                >
                    <Text className="text-black font-bold text-lg">{t('cart.proceedToCheckout')}</Text>
                </TouchableOpacity>
            </View>
        </SafeAreaView>
    );
